...
## 개선방안
- 목적과 역할에 맞는 구체적이고 명확한 변수명을 사용하여, 변수명과 적절한 주석만으로도 팀원과의 소통이 원할하게 해야하고 코드의 가독성을 향상시켜야 함
  (ERD 설계시에도 테이블간의 관계를 명확히 하고 흐름을 쉽게 알아볼 수 있는 컬럼명을 작성해야함)
- 기능의 실행에만 초점을 맞추는게 아닌, 유효성 검사를 통해 올바른 데이터가 넘어갈 수 있도록 하고 예상치 못한 오류를 방지하여 시스템의 안전성과 보안을 강화해야 함
- 역할을 분담하여 작업을 수행하였지만, 프로젝트 전체의 구조와 기능을 알 필요가 있다고 생각해 프로젝트가 끝난 후 팀원들의 코드를 리뷰하고 학슴함
